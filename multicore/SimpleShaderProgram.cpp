...
bool SimpleShaderProgram::Init(ID3D11Device* device, ID3D11DeviceContext* deviceContext, UINT backBufferWidth, UINT backBufferHeight, Console* console, ContentManager* contentManager)
{
...
bool SimpleShaderProgram::InitBuffers(ID3D11UnorderedAccessView* backBufferUAV)
{
	if(!ShaderProgram::InitBuffers(backBufferUAV))
		return false;

	LogErrorReturnFalse(sphereBuffer.Create<SphereBuffer>(device, D3D11_BIND_CONSTANT_BUFFER, D3D11_USAGE_DEFAULT, static_cast<D3D11_CPU_ACCESS_FLAG>(0), &sphereBufferData), "Couldn't create sphere buffer: ");
	LogErrorReturnFalse(triangleVertexBuffer.Create<VertexBuffer>(device, D3D11_BIND_CONSTANT_BUFFER, D3D11_USAGE_DEFAULT, static_cast<D3D11_CPU_ACCESS_FLAG>(0), &vertexBufferData), "Couldn't create triangle vertex buffer: ");
	LogErrorReturnFalse(triangleBuffer.Create<TriangleBuffer>(device, D3D11_BIND_CONSTANT_BUFFER, D3D11_USAGE_DEFAULT, static_cast<D3D11_CPU_ACCESS_FLAG>(0), &triangleBufferData), "Couldn't create triangle index buffer: ");
...
bool SimpleShaderProgram::InitShaders()
{
	//////////////////////////////////////////////////
	//Cbuffers
	//////////////////////////////////////////////////
	...
	LogErrorReturnFalse(primaryRayGenerator.CreateFromFile("PrimaryRayGenerator.hlsl", device, primaryResourceBinds0), "");
	...
	LogErrorReturnFalse(traceShader.CreateFromFile("Intersection.hlsl", device, traceResourceBinds0, traceResourceBinds1), "");
	...
	LogErrorReturnFalse(intersectionShader.CreateFromFile("Shading.hlsl", device, shadeResourceBinds0, shadeResourceBinds1), "");
	...
	LogErrorReturnFalse(compositShader.CreateFromFile("Composit.hlsl", device, compositResourceBinds0, compositResourceBinds1), "");

	return true;
}

void SimpleShaderProgram::Update(std::chrono::nanoseconds delta)
{
...
void SimpleShaderProgram::DrawRayPrimary()
{
...
void SimpleShaderProgram::DrawRayIntersection(int config)
{
...
void SimpleShaderProgram::DrawRayShading(int config)
{
...
void SimpleShaderProgram::DrawComposit(int config)
{
...
std::string SimpleShaderProgram::ReloadShadersInternal()
{
