...
{
	if(!CastStyleTo<TextFieldStyle>(style.get(), LOG_TYPE::FATAL))
		return;

	this->style = std::dynamic_pointer_cast<TextFieldStyle>(style);
	...
	{
		scrollbar.Init(this->background->GetWorkArea()
			, std::make_shared<ScrollbarStyle>()
			, this->style->scrollbarBackground
			, this->style->scrollbarBackgroundStyle
			, std::bind(&TextField::ScrollbarScrolled, this));
	}
...
void TextField::DrawBackground(SpriteRenderer* spriteRenderer)
{
	...
	DirectX::XMFLOAT2 originalDrawPos = background->GetWorkArea().GetMinPosition();

	originalDrawPos.y = background->GetWorkArea().GetMinPosition().y - yOffset;

	DirectX::XMFLOAT2 drawPos = originalDrawPos;

	for(int i = visibleStringsBegin; i < visibleStringsEnd; ++i)
	{
		spriteRenderer->DrawString(style->characterSet, std::get<0>(lines[i]), drawPos, style->textColorNormal);
...
void TextField::DrawMiddle(SpriteRenderer* spriteRenderer)
{
	if(SelectionMade())
	{
		...
		DirectX::XMFLOAT2 drawSize = DirectX::XMFLOAT2(0.0f, static_cast<float>(style->characterSet->GetLineHeight()));
		...
		for(int i = std::max(selectionStartLineIndex, visibleStringsBegin), endI = std::min(selectionEndLineIndex, visibleStringsEnd); i <= endI; ++i)
		{
			...
			unsigned int widthAtBegin = style->characterSet->GetWidthAtIndex(std::get<0>(lines[i]), begin);
			unsigned int widthAtEnd = style->characterSet->GetWidthAtIndex(std::get<0>(lines[i]), end);
			...
			spriteRenderer->Draw(Rect(drawPosition, drawSize), style->textHighlightColor);

			drawPosition.y += drawSize.y;

			drawSize = DirectX::XMFLOAT2(0.0f, static_cast<float>(style->characterSet->GetLineHeight()));
		}
	}
...
void TextField::DrawForeground(SpriteRenderer* spriteRenderer)
{
	if(drawCursor)
	{
		...
		DirectX::XMFLOAT2 drawPosition(cursorPosition);
...
{
	return GetLines(0, static_cast<int>(lines.size() - 1));
}
...
int TextField::GetCursorIndex(int width, int line) const
{
	...
	ConstructedString constructedString = style->characterSet->ConstructString(std::get<0>(lines[line]));

	return style->characterSet->GetIndexAtWidth(constructedString, width);
}

void TextField::OnKeyDown(const KeyState& keyState)
{
	if(focus)
	{
		cursorBlinkTimer.Reset();
...
void TextField::OnMouseDown(const KeyState& keyState, const DirectX::XMFLOAT2& mousePosition)
{
...
void TextField::OnMouseUp(const KeyState& keyState, const DirectX::XMFLOAT2& mousePosition)
{
	if(keyState.key == MOUSE_BUTTON::LEFT)
	{
		mouseDown = false;
...
void TextField::SetPosition(const DirectX::XMFLOAT2& newPosition)
{
...
{
	DirectX::XMVECTOR xmLhs = DirectX::XMLoadFloat2(&cursorPosition);
	DirectX::XMVECTOR xmRhs = DirectX::XMLoadFloat2(&background->GetWorkArea().GetMinPosition());

	DirectX::XMFLOAT2 diff;

	DirectX::XMStoreFloat2(&diff, DirectX::XMVectorSubtract(xmLhs, xmRhs));
	...
	xmLhs = DirectX::XMLoadFloat2(&background->GetWorkArea().GetMinPosition());
	xmRhs = DirectX::XMLoadFloat2(&diff);

	DirectX::XMStoreFloat2(&cursorPosition, DirectX::XMVectorAdd(xmLhs, xmRhs));
}

void TextField::SetSize(const DirectX::XMFLOAT2& newSize)
{
...
void TextField::SetSize(float x, float y)
{
	...
	if(xDelta != 0)
	{
		...
		scrollbar.SetMaxItems(static_cast<int>(lines.size()));
		scrollbar.SetSize(DirectX::XMFLOAT2(scrollbar.GetSize().x, y));
...
void TextField::SetCursorIndex(int newIndex)
{
	...
	cursorPosition.x = background->GetWorkArea().GetMinPosition().x + style->characterSet->GetWidthAtIndex(std::get<0>(lines[cursorLineIndex]), cursorIndex);
}
...
void TextField::MoveCursorUp()
{
	if(cursorLineIndex > 0)
	{
		...
		ConstructedString constructedString = style->characterSet->ConstructString(std::get<0>(lines[cursorLineIndex]));
		SetCursorIndex(style->characterSet->GetIndexAtWidth(constructedString, matchWidth));
	}
}
...
std::vector<std::tuple<std::string, bool>> TextField::CreateLineData(const std::string& text) const
{
	...
	for(const CharacterBlock& block : style->characterSet->ConstructString(text).characterBlocks)
	{
...
GUIStyle* TextField::GenerateDefaultStyle(ContentManager* contentManager)
{
...
GUIBackground* TextField::GenerateDefaultBackground(ContentManager* contentManager)
{
	return new OutlineBackground();
}

GUIStyle* TextField::GenerateDefaultBackgroundStyle(ContentManager* contentManager)
{
